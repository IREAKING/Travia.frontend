  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 w-14 h-14 rounded-full bg-gradient-to-r from-cyan-500 to-purple-500 text-white shadow-lg shadow-cyan-500/50 hover:shadow-cyan-500/70 transition-all duration-300 flex items-center justify-center group ${
      ...
      {/* Chat Panel */}
      {isOpen && (
        <div className="fixed bottom-24 right-2 md:right-6 z-50 w-[calc(100vw-1rem)] md:w-96 max-w-md h-[600px] max-h-[calc(100vh-8rem)] bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-5 duration-300">
          {/* Header */}
              ...
              <div className="flex items-center gap-2">
                <label className="flex items-center gap-1 text-xs text-slate-300 cursor-pointer">
                  <input
                    ...
                    className="w-3 h-3 rounded border-white/20 bg-slate-800 text-cyan-500 focus:ring-cyan-500"
                  />
                  <span>Tours</span>
                </label>
                <button
                  onClick={clearChat}
                  className="px-2 py-1 text-xs text-slate-400 hover:text-white bg-slate-800/50 hover:bg-slate-800 rounded-lg transition-colors"
                  title="Xóa chat"
                >
                  🗑️
                </button>
          ...
          <div className="border-t border-white/10 p-4 bg-slate-900/50">
            <div className="flex gap-2">
              <textarea
                ...
                className="flex-1 px-3 py-2 bg-slate-800/50 border border-white/10 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 focus:ring-2 focus:ring-cyan-500/20 transition-all resize-none text-sm"
                rows={2}
                disabled={isLoading}
              />
              <button
                ...
                className="px-4 py-2 bg-gradient-to-r from-cyan-500 to-purple-500 text-white font-semibold rounded-xl shadow-lg shadow-cyan-500/30 hover:shadow-cyan-500/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center min-w-[80px]"
              >
                {isLoading ? (
